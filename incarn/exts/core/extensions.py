...
from incarn.converters import Extension
...
UNLOAD_BLACKLIST = {
    f"{exts.__name__}.core.extensions",
    f"{exts.__name__}.core.dev",
...
class Action(Enum):
    ...
    LOAD = functools.partial(IncarnBot.load_extension)
    UNLOAD = functools.partial(IncarnBot.unload_extension)
    RELOAD = functools.partial(IncarnBot.reload_extension)
...
class Extensions(commands.Cog):
    ...
    async def list_command(self, ctx: Context) -> None:
        ...
        Cross indicates that the extension is unloaded.

        Checkmark indicates that the extension is currently loaded.
        """
        embed = Embed(colour=Colours.purple)
        # embed.set_author(
        ...
        embed.set_author(
            name="Extensions List",
            url="https://github.com/SidVeld/Incarn",
            # icon_url=URLs.bot_avatar
        )
