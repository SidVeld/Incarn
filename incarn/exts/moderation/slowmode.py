...
from incarn.constants import MODERATION_ROLES
from incarn.converters import DurationDelta
from incarn.log import get_logger
from incarn.utils import time
...
class Slowmode(Cog):
    ...
    async def set_slowmode(self, ctx: Context, channel: Optional[TextChannel], delay: DurationDelta) -> None:
        ...
        if slowmode_delay <= SLOWMODE_MAX_DELAY:
            ...
            await ctx.send(
                f':white_check_mark: The slowmode delay for {channel.mention} is now {humanized_delay}.'
            )
            ...
            await ctx.send(
                ':x: The slowmode delay must be between 0 and 6 hours.'
            )
