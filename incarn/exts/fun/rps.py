...
class RPS(commands.Cog):
    ...
    async def rps(self, ctx: commands.Context, move: str) -> None:
        ...
        if player_result == 0:
            message_string = f"{player_mention} and me played {bot_move}, it's a tie."
            await ctx.send(message_string)
        elif player_result == 1:
            await ctx.send(f"I chosed {bot_move}! {player_mention} won!")
        else:
            await ctx.send(f"I chosed {bot_move}! {player_mention} lost!")
