...
class Colour(Cog):
    ...
    def __init__(self, bot: IncarnBot):
        self.bot = bot
        with open(Path("incarn/resources/utilities/colors.json")) as file:
            self.colour_mapping: dict = json.load(file)
            del self.colour_mapping["_"]
            del self.colour_mapping["__"]
    ...
    @commands.group(aliases=("color", "clr"), invoke_without_command=True)
