...
def is_staff_channel(channel: discord.TextChannel) -> bool:
    ...
    guild = incarn.instance.get_guild(constants.Guild.id)
...
async def get_or_fetch_channel(channel_id: int) -> discord.abc.GuildChannel:
    ...
    channel = incarn.instance.get_channel(channel_id)
    if not channel:
        log.debug(f"Channel {channel_id} is not in cache; fetching from API.")
        channel = await incarn.instance.fetch_channel(channel_id)
